...
public class ErrorReporter implements Thread.UncaughtExceptionHandler {
	...
	private String createCustomInfoString() {
		...
		while (iterator.hasNext()) {
			...
			CustomInfo = CustomInfo.append(CurrentKey + " = " + CurrentVal
					+ "\n");
		}
	...
	private void retrieveCrashData(final Context context) {
		try {
			...
			// Device Memory
			mCrashProperties.put(TOTAL_MEM_SIZE_KEY,formatFileSize(getTotalInternalMemorySize()));
			mCrashProperties.put(AVAILABLE_MEM_SIZE_KEY,formatFileSize(getAvailableInternalMemorySize()));
     ...
     */
    public static String formatFileSize(final long number) {
     ...
     */
    public static String formatShortFileSize(final long number) {
        return formatFileSize(number, true);
    }

    private static String formatFileSize(final long number, final boolean shorter) {
        float result = number;
        String suffix = "B";
        if (result > 900) {
            suffix = "KB";
	...
	private void showCrashDialog(final String reportFileName) {
		final Intent intent = new Intent(mApplication,
				CrashReportDialogActivity.class);
